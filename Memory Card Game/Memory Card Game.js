...
function initField() {
  ...
  btnList = ["button1", "button2", "button3", "button4", "button5", "button6", "button7", "button8", "button9", "button10", "button11", "button12", "button13", "button14", "button15", "button16", "button17", "button18", "button19", "button20", "button21", "button22", "button23", "button24", "button25", "button26", "button27", "button28", "button29", "button30", ]
  ...
  function showImage(btn, img) {
    btn.onclick = function() {
      if (btnList.includes(btn.id)) {
        if (img.style.display !== "block") {
          if (count % 2 === 0) {
            firstImage = img;
            if (btnList.includes(btn.id)) {
              deleteFirst = btnList.indexOf(btn.id);
              btnList.splice(deleteFirst, 1);
              console.log(btnList);
            }
            ...
            secondImage = img;
            if (btnList.includes(btn.id)) {
              deleteSecond = btnList.indexOf(btn.id);
              btnList.splice(deleteSecond, 1);
              console.log(btnList);
            }
            img.style.display = "block";
            count++;
            title.innerHTML = count;
            if (firstImage.getAttribute("src") === secondImage.getAttribute("src")) {} else {
              setTimeout(function() {
